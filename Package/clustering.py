...
def adj(c_in,c_out,theta, fraction):

    ''' This function creates the adjacency matrix for two classes, according to the DC-SBM model 

    Use: A = adj(c_in,c_out,theta, fraction)
    Inputs: c_in (scalar), c_out (scalar), theta (vector) are the parameters of the DC-SBM model. fraction is the size of the two classes. For even sizes fraction = n/2
    ...
    c_matrix = np.multiply(theta*np.ones((n,n))*theta[:, np.newaxis],c_matrix) # introduce the \theta_itheta_j dependence
        ...
        if d[i] == 0:
            ...
            if first_node > int(fraction): # if the element is in the second family
                ...
                if r < (c_in/(c_in+c_out)): # if it connects with an element of the same family
                    second_node = np.random.choice(np.arange(fraction,n), p=theta[fraction:]/sum(theta[fraction:])) # second node is in the second family
                else:
                    second_node = np.random.choice(np.arange(fraction), p=theta[0:fraction]/sum(theta[0:fraction])) # second node is in the first family
            else:
                r = np.random.uniform(0,1)
                if r < (c_out/(c_in+c_out)): # if the first element is in the first family and the second is in the same
                    second_node = np.random.choice(np.arange(fraction,n), p=theta[fraction:]/sum(theta[fraction:])) # second node is in the first family
                else:
                    second_node = np.random.choice(np.arange(fraction), p=theta[0:fraction]/sum(theta[0:fraction])) # second node is in the second family
...
def BH(A,classes,r,assortativity):
    ...
    ''' This function performs spectral clustering on the second (resp. first) eigenvector of the Bethe Hessian H_r, if one seeks assortative (resp. disassortative) blocks. 
    ...
    Outputs: y_kmeans the vector of size n of detected classes, eigenvalues the values of the two smallest eigenvalues of H_r (algebraic), X the eigenvector of size n used for the classification and precision the overlap between the detected classes y_kmeans and the ground truth classes.

    '''

    n = len(A)
    H = (r**2-1)*np.diag(np.ones(n)) + np.diag(np.sum(A,axis = 0)) - r*A
    H = H.astype(float)
    d = np.sum(A.astype(float),axis = 0) # degree vector
    eigenvalues,eigenvectors = scipy.sparse.linalg.eigsh(H, 2, which='SA') # computation of the smallest eigenvalues
...
def adj_cluster(A,classes, assortativity):

    ''' This function performs spectral clustering on the second largest (resp. smallest) eigenvector of the adjacency matrix A, if one seeks assortative (resp. disassortative) blocks. 

    Use: y_kmeans,eigenvalues, X, ov = adj_cluster(A,classes, assortativity)
    Inputs: A a (symmetric) n x n adjacency matrix of a graph, classes a vector of size n with the underlying ground truth class assignment and assortativity is set to 1 if one seeks assortative blocks or to -1 if one seeks disassortative blocks. 
    ...
    n = len(A)
    number_eig = 2 # number of eigenvalues to compute
    A = A.astype(float)
    if assortativity > 0:
        eigenvalues,eigenvectors = scipy.sparse.linalg.eigsh(A, number_eig, which='LA')
    else:
        eigenvalues,eigenvectors = scipy.sparse.linalg.eigsh(A, number_eig, which='SA')
    ...
    kmeans.fit(X)
    y_kmeans = kmeans.predict(X) # do kmeans on the informative eigenvector
...
def sym_lap(A,classes, assortativity):

    ''' This function performs spectral clustering on the second largest (resp. smallest) eigenvector of the symmetric laplacian matrix D^{-1/2}AD^{-1/2}, if one seeks assortative (resp. disassortative) blocks. 

    Use: y_kmeans,eigenvalues, X, ov = sym_lap(A,classes, assortativity)
    Inputs: A a (symmetric) n x n adjacency matrix of a graph, classes a vector of size n with the underlying ground truth class assignment and assortativity is set to 1 if one seeks assortative blocks or to -1 if one seeks disassortative blocks. 
    Outputs: y_kmeans the vector of size n of detected classes, eigenvalues the values of the two smallest (largest) eigenvalues of A, X the eigenvector of size n used for the classification and ov the overlap between the detected classes y_kmeans and the ground truth classes.
    ...
    if assortativity > 0:
        eigenvalues,eigenvectors = scipy.sparse.linalg.eigsh(L, number_eig, which='LA')
    else:
        eigenvalues,eigenvectors = scipy.sparse.linalg.eigsh(L, number_eig, which='SA')
...
def lap(A,classes, assortativity):

    ''' This function performs spectral clustering on the second largest (resp. smallest) eigenvector of the random walk laplacian matrix D^{-1}A, if one seeks assortative (resp. disassortative) blocks. 

    Use: y_kmeans,eigenvalues, X, ov = lap(A,classes, assortativity)
    Inputs: A a (symmetric) n x n adjacency matrix of a graph, classes a vector of size n with the underlying ground truth class assignment and assortativity is set to 1 if one seeks assortative blocks or to -1 if one seeks disassortative blocks. 
    Outputs: y_kmeans the vector of size n of detected classes, eigenvalues the values of the two smallest (largest) eigenvalues of A, X the eigenvector of size n used for the classification and ov the overlap between the detected classes y_kmeans and the ground truth classes.
    ...
    kmeans = KMeans(n_clusters = 2)
    kmeans.fit(X)
    y_kmeans = kmeans.predict(X) # do kmeans on the relevant eigenvector
...
def find_r(A,n_cycles,assortativity):
    ...
    phi = np.mean(d**2)/c**2 # estimation of the second moment of the \theta
...
def find_r_inside(A,n_cycles,assortativity):
    ...
    d = np.sum(A,axis = 0)
    c = np.mean(d) # average degree
    phi = np.mean(d**2)/c**2 # estimate of the second moment of \theta
    if assortativity > 0:
